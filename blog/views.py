...
def get_blog_list_common_data(request, blogs_all_list):
    ...
    page_range = [x for x in range(current_page_num-2, current_page_num + 2) if x in paginator.page_range]
    # 加上省略代码标记
    if page_range[0] - 1 > 1:
        page_range.insert(0,"...")
...
def blog_detail(request, blog_pk):
    ...
    response.set_cookie(read_cookie_key, 'true', max_age=60)   # 阅读cookie的标记
    # response.set_cookie('blog_%s_read' % blog_pk, 'true', expires=60)
    return response
